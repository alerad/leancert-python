...
def test_float_and_truth_rejected():
    x = lc.var("x")
    with pytest.raises(lc.InexactFloatError):
        lc.const(0.1)
...
def test_normalization_equivalences():
    ...
    assert lc.semantically_equal(x >= 0, x >= 0)
    assert lc.normalize(lc.sin(0)) == lc.const(0, lc.REAL)


def test_identity_domain_and_metadata():
    x = lc.var("x", namespace="a")
    y = lc.var("x", namespace="b")
    assert x != y
    b1 = lc.box({x: (0, 1), y: (0, 2)})
    b2 = lc.box({y: (0, 2), x: (0, 1)})
    assert lc.semantic_digest(b1) == lc.semantic_digest(b2)
    assert lc.semantic_digest(
        lc.Annotated(x, {"span": lc.SourceSpan("a.py", 1, 2)})
    ) == lc.semantic_digest(x)
...
def test_strict_duplicate_key_and_version():
    with pytest.raises(lc.AstDecodeError):
        lc.decode_canonical('{"schema":"leancert.ast","schema":"x","version":1,"root":{}}')
    with pytest.raises(lc.UnknownAstNodeVersion):
        lc.decode_canonical({"schema": "leancert.ast", "version": 2, "root": {}})


def test_external_authoritative_identity():
    p = lc.PackageIdentity("pkg", "git", "abc")
    f = lc.external_unary("Pkg.f", p, "pkg.f.v1")
    with pytest.raises(lc.UnresolvedExternalIdentityError):
        lc.semantic_digest(lc.FunctionCall(f, (lc.var("x"),)))
...
def test_named_constants_transcendentals_and_derivative():
    ...
    assert lc.alpha_equivalent(lc.decode_canonical(lc.encode_canonical(derivative)), derivative)
